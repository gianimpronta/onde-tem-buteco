...
export function ButecosFilterForm({ cidadeOptions, bairroOptions }: Props) {
  ...
  return (
    <form className="grid gap-4 sm:grid-cols-2 lg:grid-cols-[1.2fr_1fr_1fr_auto_auto] lg:items-end">
      ...
      <Link
        href="/butecos"
        className="inline-flex items-center justify-center rounded-full border border-tinto-700 px-4 py-2 text-[13px] font-medium text-tinto-700 transition hover:bg-terracota-100 dark:border-brand dark:text-brand dark:hover:bg-surface-alt"
      >
        Limpar
      </Link>
    </form>
